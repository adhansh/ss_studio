...
def switch():
    ...
    if bIsColor:
        SetMono()
        btnSwitch.config(image = off)
        bIsColor = False
    else:
        SetColor()
        btnSwitch.config(image = on)
        bIsColor = True    
...
#    btnColor = Button(frame_bot, text = '컬러촬영\n(사용불가)', font=('MALGUN', 40), background='magenta', command = SetColor, width=10, height=3, state='disabled')
#    btnMono.grid(row=3, column=1)
#    btnColor.grid(row=3, column=3)
# 스위치 형태로 변경
    lebelSwitch = Label(frame_bot, text='컬러 촬영 모드 설정', font=('Helvetica', 20), fg='grey30')
    btnSwitch = Button(frame_bot, image = off, bd = 0, command = switch)
    lebelSwitch.grid(row=3, column=1)
    btnSwitch.grid(row=3, column=3)
