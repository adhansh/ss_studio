...
BURST_SHOT_MAX = 10     # 총 버스트모드가 사진찍는 횟수
...
def BurstShotThreadFunc(context):
    ...
    gIsThreading = True
    #############################################################################
    # start tkinter transperancy wnd
    root = Tk()
...
def StartBurstMode():
    ...
    if gIsThreading:
        print('aready burst thread is exsist\n')
        return False
    else:
        gStopBurst = False
...
def burstCmdJob(cmd):    
    bRet = True
    if 'cmd:startburst' == cmd:
        print('burst cmd(start)')
        bRet = StartBurstMode()
        
    elif 'cmd:stopburst' == cmd:
        print('burst cmd(stop)') 
        bRet = StopBurstMode()
    return bRet                      
